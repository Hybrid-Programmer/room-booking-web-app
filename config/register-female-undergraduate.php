<?php
  ...
  $response = '';
    ...
    $level = mysqli_real_escape_string($conn, $_POST['level']);
      ...
      if(!empty($membership) && !empty($firstname) && !empty($middlename) && !empty($lastname) && !empty($email) && !empty($contact) && !empty($gender) && !empty($age) && !empty($marital_status) && !empty($address)  && !empty($parish)  && !empty($roomNumber)  && !empty($academics) && !empty($institution) && !empty($level)){

        if(filter_var($email, FILTER_VALIDATE_EMAIL)){

            $sql = mysqli_query($conn, "SELECT email FROM female_users WHERE email = '$email'");
                ...
                if(isset($_FILES['profile']['name'])){
                    ...
                    if(in_array($img_ext, $extensions) === true){
                            ...
                            if(move_uploaded_file($tmp_name, "$targetDir/".$newName)){ //Move the uploaded image to this permanent folder on the server
                                ...
                                $insert_query = mysqli_query($conn, "INSERT INTO female_users (membership, profile, firstname, middlename, lastname, email, contact, gender, marital_status, address, parish, academic_status, school_attended, level, qualification, registered_on, booked_roomID)

                                 VALUES ('$membership', '$newName', '$firstname', '$middlename', '$lastname', '$email', '$contact', '$gender', '$marital_status', '$address', '$parish', '$academics', '$institution', '$level', 'null', '$date', '$roomNumber')");
                                ...
                                if($insert_query === true){
                                     ...
                                     $response = "You have registered successfully";
                                      ...
                                      mysqli_query($conn, "INSERT INTO notification (title, action, created) VALUES ('User registration', 'A new user registered on the website.', '$date')");
                                       ...
                                       mysqli_query($conn, "UPDATE female_rooms SET status = 'booked', booked_by = '$fullname', booking_date = '$date' WHERE roomID = '$roomNumber'");
