<?php
  ...
  $response = '';
    ...
    $academics = mysqli_real_escape_string($conn, $_POST['academics']);
      ...
      if(!empty($membership) && !empty($firstname) && !empty($middlename) && !empty($lastname) && !empty($email) && !empty($contact) && !empty($gender) && !empty($age) && !empty($marital_status) && !empty($address)  && !empty($parish)  && !empty($roomNumber)){
                    ...
                    if(in_array($img_ext, $extensions) === true){
                            ...
                            if(move_uploaded_file($tmp_name, "$targetDir/".$newName)){ //Move the uploaded image to this permanent folder on the server
                                ...
                                $insert_query = mysqli_query($conn, "INSERT INTO female_users (membership, profile, firstname, middlename, lastname, email, contact, gender, marital_status, address, parish, academic_status, school_attended, level, qualification, registered_on, booked_roomID)
                                 ...
                                 VALUES ('$membership', '$newName', '$firstname', '$middlename', '$lastname', '$email', '$contact', '$gender', '$marital_status', '$address', '$parish', '$academics', 'null', 'null', 'null', '$date', '$roomNumber')");
